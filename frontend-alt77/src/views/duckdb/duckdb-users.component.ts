              ...
              <div class="form-row">
                <div class="form-group">
                  ...
                  <select
                    ...
                    (ngModelChange)="updateFormData('role', $event)"
                    name="role"
                  >
...
export class DuckdbUsersComponent implements OnInit {
  ...
  formData = signal<Partial<User>>({ name: '', email: '', role: 'User', status: 'Active' });
  ...
  async saveUser(): Promise<void> {
    ...
    try {
      ...
      // Format: id:name:email:role:status
      if (this.editingUser()) {
        await this.api.callOrThrow('updateUser', [
          ...
          data.status || this.editingUser()!.status
        ]);
