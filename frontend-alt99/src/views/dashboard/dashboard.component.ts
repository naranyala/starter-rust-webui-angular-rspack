 ...
 * Dashboard Component
 *
 * Main dashboard with statistics and navigation to documentation and demo views
 */
      ...
      <main class="panel-second" [class.visible]="isMobileView() && showContent()">
        ...
        <div class="content-area" #contentArea>
          @if (activeView() === 'demo_sqlite_crud') {
            <markdown
              [src]="'assets/docs/demo_sqlite_crud.md'"
              (load)="onMarkdownLoad($event)"
              (error)="onMarkdownError($event)">
            </markdown>
          } @else if (activeView() === 'demo_duckdb_crud') {
            <markdown
              [src]="'assets/docs/demo_duckdb_crud.md'"
              (load)="onMarkdownLoad($event)"
              (error)="onMarkdownError($event)">
            </markdown>
          } @else if (activeView() === 'demo_checklist') {
            <markdown
              [src]="'assets/docs/demo_checklist.md'"
              (load)="onMarkdownLoad($event)"
              (error)="onMarkdownError($event)">
            </markdown>
          } @else if (activeView() === 'demo_websocket') {
            <markdown
              [src]="'assets/docs/demo_websocket.md'"
              (load)="onMarkdownLoad($event)"
              (error)="onMarkdownError($event)">
            </markdown>
          } @else if (activeView() === 'demo_features') {
            <markdown
              [src]="'assets/docs/demo_features.md'"
              (load)="onMarkdownLoad($event)"
              (error)="onMarkdownError($event)">
            </markdown>
          } @else {
...
export class DashboardComponent implements OnInit {
  ...
  activeView = signal<string>('00-index');
  ...
  docItems = signal<NavItem[]>([
    { id: '00-index', label: 'Documentation Index', icon: '📚', active: true },
    { id: '01-overview', label: 'Project Overview', icon: '🚀', active: false },
    { id: '02-architecture', label: 'Architecture', icon: '🏗️', active: false },
    { id: '03-changelog', label: 'Changelog', icon: '📝', active: false },
    { id: '04-build-system', label: 'Build System', icon: '🔨', active: false },
    { id: '05-development-workflow', label: 'Development', icon: '💻', active: false },
  ]);
  ...
  demoItems = signal<NavItem[]>([
    ...
    { id: 'demo_checklist', label: 'Interactive Checklist', icon: '✅', active: false },
    { id: 'demo_websocket', label: 'WebSocket Demo', icon: '🔌', active: false },
    { id: 'demo_features', label: 'Feature Checklist', icon: '📋', active: false },
  ]);

  currentMarkdownPath = signal('assets/docs/00-index.md');
